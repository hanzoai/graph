...
import (
	...
	"github.com/hanzoai/graph/protos/pb"
	"github.com/hanzoai/graph/schema"
	"github.com/hanzoai/graph/tok"
	"github.com/hanzoai/graph/tok/hnsw"
	"github.com/hanzoai/graph/types"
	"github.com/hanzoai/graph/x"
...
func (r *rebuilder) Run(ctx context.Context) error {
	...
	tmpStream.Send = func(buf *z.Buffer) error {
		return buf.SliceIterate(func(slice []byte) error {
			kv := &bpb.KV{}
			// The store's own types carry their own codec. `bpb.KV` is zapdb's,
			// and zapdb serializes with ZAP rather than protobuf, so it is
			// unmarshalled by the package that defines it. Every OTHER type in
			// this file is Hanzo's own protobuf and still goes through `proto`.
			if err := bpb.Unmarshal(slice, kv); err != nil {
				return err
			}
